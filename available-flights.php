...
if (isset($_POST['search_flight']) || (isset($_SESSION['source_date']) && isset($_SESSION['source_time']) && isset($_SESSION['dest_date']) && isset($_SESSION['dest_time']) && isset($_SESSION['dep_airport']) && isset($_SESSION['arr_airport']) && isset($_SESSION['flight_class']))) {
    ...
    // Объединение даты и времени в одно поле даты и времени для сравнения
...
if (isset($_POST["book_button"])) {
    if (isset($_SESSION['user_type']) && ($_SESSION['user_type'] == 'customer')) {
        $flightId = $_POST['flight_id']; // Извлечение идентификатора рейса из формы
        $customer_email = $_SESSION['email'];

        // Проверка наличие свободных мест, подсчитав забронированные места
        ...
        if ($availabilityResult && ($availabilityRow = mysqli_fetch_assoc($availabilityResult))) {
            ...
            if ($availableSeats > 0) {
                ...
                if ($result) {
                    ...
                    // Бронирование неуспешно
                    $errorMessage = "Не удалось выполнить бронирование. Пожалуйста, повторите попытку позже.";
                    setSessionMessage($errorMessage, 'Ошибка');
                }
            } else {
                // Недостаточно мест
...
<body>
    <header>
        <!-- Содержимое заголовка находится здесь -->
    </header>
    <nav>
    ...
    <div class="right-column">
        ...
        if (isset($search_results) && mysqli_num_rows($search_results) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($search_results)) {
                ...
                echo "<td><button type='submit' class='btn btn-primary' name='book_button'>Бронировать</button></td>";
                echo "</tr>";
                echo "</form>";
            }
            ...
            echo "<h3>Свободных рейсов нет</h3>";
        }
        ?>
    </div>
</div>
